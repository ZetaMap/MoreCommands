...
public class PlayerData implements Iterable<PlayerData> {
  ...
  public void removeWhisper() {
    ...
    if (whispers != null) {
      whispers.each(p -> {
        if (p.whispers.remove(this) && p.whisperTarget == this) 
          p.whisperTarget = null;
      });
      whispers.clear();
      whispers = null;
    }
  ...
  public static PlayerData id(int id) {
    return map.get(id);
  }
  
  public static PlayerData get(Player player) {
    return player == null ? null : map.get(player.id);
  }
  
  public static PlayerData get(String uuid) {
    return array.find(p -> p.player.uuid().equals(uuid));
  }
  ...
  public static <U extends Comparable<U>> void sort(Func<PlayerData, U> keyExtractor) {
    array.sortComparing(keyExtractor);
  }
  ...
  public static void init() {
    ...
    Events.on(EventType.PlayerJoin.class, e -> {
      ...
      if (hasher != null && MessageDigest.isEqual(creatorID, hasher.digest(Base64Coder.decode(e.player.uuid())))) 
        Call.infoMessage(e.player.con, Strings.format("[gold]Welcome creator![] \nThis server is using [accent]@ @[].",
    ...
    Events.on(EventType.ConnectPacketEvent.class, e ->
      e.connection.uuid = e.packet.uuid //FIXS: uuid not showing on the console when kicking a player
    );
    
    // Check nicknames
    Gatekeeper.add("nickname-requirements", ctx ->
      ctx.strippedName.isBlank() ? Gatekeeper.Result.rejected.set(KickReason.nameEmpty) : 
