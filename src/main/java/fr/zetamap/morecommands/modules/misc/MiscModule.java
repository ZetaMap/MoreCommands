...
public class MiscModule extends AbstractModule {
  ...
  public void exitServer(int status) { 
    ...
    CommandHandler.CommandResponse result = ModuleFactory.serverCommands.handler.handleMessage("exit");
  ...
  public void initImpl() {
    reconnectOnExit = new Config("reconnectOnExit", 
    ...
    Reflect.set(Vars.net, "provider", new ArcNetProvider() {
      // Copy fields from old provider
      { new Json().copyFields(Reflect.<Net.NetProvider>get(Vars.net, "provider"), this, true); }
  ...
  public void registerServerCommands(ServerCommandHandler handler) {
    ...
    handler.add("speed", "[value]", "Control the game speed. &lrUSE WITH CAUTION!&fr", args -> {
      if (args.length == 0) logger.info("Current game speed: @", gameSpeed);
        ...
        logger.warn("This can only work correctly if the clients support the 'gamespeed' packet.");
      }
    });
      ...
      if (Vars.state.isPlaying()) {
        if (args.length == 1) {
          try {
            ...
            Call.sendMessage("[gold]-------------------- \n"
                           + "[scarlet]/!\\[][orange] Current gamemode has been changed to [accent]" + args[0] + "[].\n"
                           + "[]--------------------");
  ...
  public void registerClientCommands(ClientCommandHandler handler) {
    ...
    handler.addAdmin("gamemode", "[name]", "Change the current map gamemode.", (args, player) -> {
      if (args.length == 1) {
        try {
          ...
          Call.sendMessage("[gold]-------------------- \n"
                         + "[scarlet]/!\\[][orange] Current gamemode has been changed to [accent]" + args[0] + "[] by "
    ...
    handler.addAdmin("pause", "<on|off>", "Toggle the game state.", (args, player) -> {
      ...
      if (old == news) return;
      Call.sendMessage("[gold]-------------------- \n"
                     + "[scarlet]/!\\[][orange]Game " + (pause ? "paused" : "unpaused") + "[] by "
                     + player.getName() + "[orange].\n"
                     + "[gold]--------------------");
    });
  }
