...
public class EffectsModule extends AbstractSaveableModule {  
   ...
   * These effects does nothing now, if not used correctly, but it's better to keep them disabled.
   */
  ...
  public Effects get(int id) {
    return id < 0 || id >= all.size ? null : all.get(all.orderedKeys().get(id));
  }
  
  public Effects get(Effect effect) {
    return all.get(all.orderedKeys().get(effect.id));
  }
  ...
  public void sort() {
    all.orderedKeys().sort(n -> all.get(n).id);
  }  
  ...
  protected void saveImpl(JsonSettings settings) {
    settings.put("disabled", String.class, all.orderedKeys().select(e -> all.get(e).disabled));
    settings.put("admin-only", String.class, all.orderedKeys().select(e -> all.get(e).adminOnly));
  }
    ...
    args -> {
      if (args.length == 0) {
        ...
        logger.info("Effects list: [total: @, enabled: @, admins: @]", all.size, 
                    all.orderedKeys().count(n -> !all.get(n).disabled()), 
                    all.orderedKeys().count(n -> all.get(n).adminOnly()));
    ...
    (args, player) -> {
      ...
      if (args.length == 0 || (stop && args.length == 1)) {
        ...
        Players.info(player, "[orange]---- [gold]Effect list [lightgray]@[gray]/[]@ [gray]([]@[gray])[][][] ----", 
                     page, pages, player.admin() ? all.size : everyoneEffects.size);
      ...
      selector.execute((p, u) -> {
        ...
        else Players.ok(p, "Particle effect [accent]@[] ([accent]@[]) started by @[green].", effect.name, effect.id,
                        player.getName()); 
      });
      if (selector.noTargetFound()) Players.ok(player, "No player was selected.");
      else if (stop) Players.ok(player, "Removed particle effect @[green].",  selector.formatMessage("from", true));
