...
public class WorldEditModule extends AbstractModule {
  ...
  public void sendBlockSnapshot() {
    try {
      ...
      for (Tile tile : Vars.world.tiles) {
        ...
        dataStream.writeInt(tile.build.pos());
        dataStream.writeShort(tile.build.block.id);
  ...
  public void sendEntitySnapshot() {
    try {
      for (Player p : Groups.player)
        Vars.netServer.writeEntitySnapshot(p);
    } catch (Exception e) {
      logger.err("Failed to send entity snapshot. Resending world data to players...");
      logger.err(e);
      sendWorld();
    }
  }
  ...
  public void sendEntitySnapshot(Player player) {
    try {
      Vars.netServer.writeEntitySnapshot(player);
    } catch (Exception e) {
  ...
  protected void initImpl() {
    // Load planet specific cores
    Vars.content.planets().each(p -> p.defaultCore.techNode != null && !coresEvolutions.containsKey(p.defaultCore), p -> {
  ...
  public void registerClientCommands(ClientCommandHandler handler) {
    ...
    handler.addAdmin("place", "<blockName> [player|x,y] [teamName|~] [buildData...]", "Place a block.", (args, player) -> {
      ...
      if (block == Blocks.air) 
        ...
        Players.ok(player, "Built @ [accent]@[] at [accent]@,@[] for the [white]@[] team.", pronoun, block.name, tile.x, 
                   tile.y, team.coloredName()); 
      else 
    ...
    (args, player) -> {
      ...
      if (block == Blocks.air) {
        ...
        if (block.hasBuilding())
          Players.ok(player, "Built [accent]@ @[] from [accent]@,@[] to [accent]@,@[] for the [white]@[] team.", count, 
                     block.name, x, y, xn, yn, team.coloredName());  
        else 
          Players.ok(player, "Placed [accent]@ @[] blocks from [accent]@,@[] to [accent]@,@[].", count, block.name, x, y, xn, 
                     yn);
      }
    ...
    handler.addAdmin("core", "<small|medium|big|coreName> [player|x,y] [teamName|~...]", "Build a core.", (args, player) -> {
      if (player.vanished()) {
        Players.err(player, "You can't build a core in vanish mode!");
        return;
      }
    ...
    handler.addAdmin("spawn", "<unit> [count] [player|x,y] [teamName|~] [unitData...]", "Spawn a unit.", (args, player) -> {
    ...
    handler.addAdmin("transform", "<unit> [player|selector] [unitData...]", "Transform a player unit.", (args, player) -> {
      UnitType unit = Vars.content.unit(Strings.kebabize(args[0]));
      if (unit == null) {
        Players.err(player, "No unit named '[orange]@[]' found.", args[0]);
        return;
      ...
      if (args.length == 1) {
        ...
        else Players.err(player, "Unable to transform you to @ [orange]@[].", pronoun, unit.name);
        return;
      }
      ...
      selector.execute((p, u) -> {
        ...
        Players.warn(p, "You have been transformed to @ [accent]@[] by @[orange].", pronoun, unit.name, player.getName());
      });
      ...
      if (invalids > 0) 
        Players.err(player, "Unable to transform [orange]@ players[] to @ [orange]@[].", invalids, pronoun, unit.name);
    ...
    handler.addAdmin("clear-map", "[hard|y|n]", "Kill all units and blocks, except cores, on the map.", (args, player) -> {
      if (cleanupTriggerer != null) {
        if (cleanupTriggerer != player) {
          ...
          Players.err(player, "Please confirm, or not, the operation. Use [orange]/clear-map y[] or [orange]/clear-map n[].");
          return;
        }  
    ...
    handler.addAdmin("weather", "[clear|weatherName] [intensity] [inf|duration]", "Control map weather.", (args, player) -> {
      if (args.length == 0) {
      ...
      if (duration == Float.POSITIVE_INFINITY) 
        Players.ok(player, "Weather [accent]@[] created with [accent]@%[] of intensity [accent]forever[].", weather.name, 
                   intensity);
      else 
        Players.ok(player, "Weather [accent]@[] created with [accent]@%[] of intensity for [accent]@[].", weather.name, 
                   intensity, DurationFormatter.format((long)(duration * 1000)));
    });
  }
}
