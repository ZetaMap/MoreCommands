...
public class Punishment {
  ...
  /**
   * Only {@code null} when it's an {@link #address} punishment.
   * {@link #target} and {@link #address} cannot be both {@code null}.
   */
  ...
  Punishment(int id, String author, String target, String address, Type type, long creation, long expire,
             String reason, Pardon pardon) {
    this.id = id;
    this.author = author;
    this.target = target;
  ...
  protected void check() {
   if (target == null && address == null)
     throw new IllegalArgumentException("Invalid punishment '" + id +
                                        "': target and address cannot not be both null.");
  }
  ...
  /** @return a negative value if punishment is expired, {@link Long#MAX_VALUE} if punishment is permanant,
   * else the remaining time */
  public long remaining() {
    return permanant() ? Long.MAX_VALUE : expire - Time.millis();
  }
  ...
  public enum Type {
    ...
    public boolean thisType(Punishment p) {
      return this == p.type;
    }
  }
