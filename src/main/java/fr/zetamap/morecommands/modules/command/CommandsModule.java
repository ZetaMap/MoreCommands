...
public class CommandsModule extends AbstractSaveableModule {
  ...
  protected void addCommand(Seq<Command> commands, ObjectMap<String, Command> handlerCommands, 
    ...
    if (c == null) return;
    handlerCommands.put(command, handlerOrderedCommands.add(c).peek());
  }
  ...
  public boolean isClientAdminCommand(String command) {
    return ModuleFactory.clientCommands.isAdmin(command);
  }
  ...
  protected void loadImpl(JsonSettings settings) {
    ...
    // This remove all commands that have not been created by MoreCommands
    ...
    if (settings.getBool("demomode")) {
      ...
      Core.app.post(() -> {
        Vars.netServer.clientCommands.getCommandList().copy().each(
          c -> !ModuleFactory.clientCommands.all.contains(c),
          c -> ModuleFactory.clientCommands.remove(c.text));  
        ServerControl.instance.handler.getCommandList().copy().each(
          c -> !ModuleFactory.serverCommands.all.contains(c),
          c -> ModuleFactory.serverCommands.remove(c.text));
      });
    }
  }
  ...
  public void registerServerCommands(ServerCommandHandler handler) {
    handler.add("commands", "[list|reset|on|off] [command] [now]", "Toggle client/server commands.", 
    args -> {
      if (clientCommands == null || clientCommands == null) {
        ...
        Seq<Command> players = ModuleFactory.clientCommands.all.select(c -> !ModuleFactory.clientCommands.admin.contains(c));
        if (players.any()) {
        ...
        if (ModuleFactory.clientCommands.admin.any()) {
          logger.info("Admin commands: [total: @]", ModuleFactory.clientCommands.admin.size);
          ModuleFactory.clientCommands.admin.each(c -> 
            logger.info(Strings.format("&lk|&fr &b&lb@@@@&fr - &lw@&fr", clientHandler.prefix, c.text, 
                                       c.paramText.isEmpty() ? "" : " &lc&fi", c.paramText, 
                                       Strings.normalize(c.description))));
        ...
        if (ModuleFactory.serverCommands.all.any()) {
          logger.info("Server commands: [total: @]", ModuleFactory.serverCommands.all.size);
          ModuleFactory.serverCommands.all.each(c -> 
