...
public class Gatekeeper {
  ...
  protected static final Timekeeper timer = new Timekeeper(60f); // cleanup every minutes
  
  public static long defaultCooldownDuration = 5 * 1000;
  ...
  public static Processor get(String name) {
    for (Priority p : Priority.all) {
      Processor proc = p.processors.get(name);
  ...
  public static boolean has(Processor proc, Priority priority) {
    return priority.processors.containsValue(proc, true);
  }
  ...
  public static boolean has(String name, Priority priority) {
    return priority.processors.orderedKeys().contains(name);
  }
  ...
  public static Result process(Context context) {
    ...
    for (Priority p : Priority.all) {
      for (String name : p.processors.orderedKeys()) {
        total++;
        try {
          Result result = p.processors.get(name).process(context);
          Events.fire(processEvent.set(name, p, result));
          if (result instanceof Result.Rejected) return result;
          
        } catch (Exception e) { 
          failed++;
          logger.err("Processor @ failed to verify player '@' [@, @].", name, context.name, context.address, context.uuid);
          logger.err(e);
        }
      }      
    }
  ...
  public static void init() {
    Events.on(EventType.ConnectPacketEvent.class, e -> {
      ...
      if (lastKick != null && lastKick > Time.millis()) {
      ...
      if (!(result instanceof Result.Rejected)) return;
      
      Result.Rejected reject = (Result.Rejected)result;
  ...
  public static class Context {
    ...
    Context set(int version, String versionType, Seq<String> mods, String name, String locale, String uuid, String usid, 
                String address, boolean mobile, int color) {
  ...
  public static enum Priority {
    ...
    final OrderedMap<String, Processor> processors = new OrderedMap<>();
  }
}
